...
var $nav_secondary = $('[data-object="nav-secondary"]'),
  ...
  updateActiveNav = function() {
    var scroll_position = $(document).scrollTop(),
      ...
      $links = $nav_secondary.find(
        '[data-behavior="nav-secondary.scroll-to-top nav-secondary.make-active"]'
      );
    ...
    if (fixed_nav) {
      $links.each(function() {
        ...
        if (
          $section.position().top <= scroll_position &&
...
$nav_secondary.on("nav-secondary.scroll-to-top", function(event, opts) {
  ...
  if (opts.object.hasClass("is-fixed-nav")) {
    if ($(window).scrollTop() < opts.object.height()) {
      ...
      offset_height = opts.object.height() * 2.5;
      window.console.log("offset_height ADJUSTED" + offset_height);
    } else {
      offset_height = opts.object.height() + 15;
      window.console.log("offset_height" + offset_height);
    }
  }
...
$(window).scroll(
  $.throttle(250, function() {
    $fixed_nav.toggleClass(
      "is-fixed",
      $(this).scrollTop() > $fixed_nav.height()
    );
    updateActiveNav();
  })
);
